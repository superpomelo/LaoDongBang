...
//            NSString *JsonStr = [JsonString convertToJsonData:parameters];
    [BaseRequestDatas uploadImageDataWithPath:path parameters:parameters imgData:parameters HTTPMethod:HTTPMethodPOST HeaderType:HeaderTypeContentTypeTOKEN mimeType:@"video/mp4" fileName:@"file.mp4" isShowLoading:NO success:^(id  _Nonnull result) {
        NSLog(@"%@",result);
        NSDictionary *dict = [NSDictionary dictionary];
        dict = result;
