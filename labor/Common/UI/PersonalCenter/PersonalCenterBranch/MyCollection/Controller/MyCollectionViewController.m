...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        /**0课程 1新闻 2圈子*/
        if (self.type == 0) {
            CollectionKeChengModel *mode = self.kechengArray[indexPath.row];
            [self requestsyscollectmycollectdele:mode.idx];
        }else if (self.type == 1){
            CollectionXinWenModel *mode = self.xinwenArray[indexPath.row];
            [self requestsyscollectmycollectdele:mode.idx];
        }else if (self.type == 2) {
            CollectionquanziModel *mode = self.quanziArray[indexPath.row];
            [self requestsyscollectmycollectdele:mode.idx];
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.type == 0) {
        ...
        if (self.xinwenArray.count == 0) {
            ...
            CollectionXinWenModel *model = self.xinwenArray[indexPath.row];
...
//    dict[@""] = @"";
    [PersonalCenterRequestDatas syscollectpagerequestDataWithparameters:nil success:^(id  _Nonnull result) {
        ...
        if ([dict.allKeys containsObject:@"listlesson"]) {
         self.kechengArray =  [CollectionKeChengModel mj_objectArrayWithKeyValuesArray:dict[@"listlesson"]]; //数组
        }
        if ([dict.allKeys containsObject:@"list1Info"]) {
         self.xinwenArray =  [CollectionXinWenModel mj_objectArrayWithKeyValuesArray:dict[@"list1Info"]]; //数组
        }
